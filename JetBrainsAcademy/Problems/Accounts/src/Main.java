...
class Main {
    ...
    public static void printFilteredAccounts(List<Account> accounts) {
        final long tooMuchMoneyAmount = 100_000_000;
        List<Account> nonEmptyAccounts = filter(accounts, account -> account.getBalance() > 0);
        List<Account> accountsWithTooMuchMoney = filter(accounts, account ->
                !account.isLocked() && account.getBalance() >= tooMuchMoneyAmount);
